...
peePodcastTrack::peePodcastTrack(peePodcast *pParent,tm* date, const char* title,const char* htmlPath,int size) {
	...
	_localPath=(char*)malloc(strlen(PODCAST_DIR)+strlen(pParent->_directory)+strlen(strtime)+strlen(_title)+50);
	_localPath[0]=0;
	//sprintf (_localPath,"%s/",DOWNLOAD_ROOT_DIR);
	//sprintf (_localPath,"podcast/");
	sprintf (_localPath,"%s/",pParent->_directory);
